...
interface Resource {
  id: string;
  title: string;
  type: "video" | "pdf" | "ppt" | "folder" | "url" | "link" | "zip" | "reference";
  fileUrl?: string | { base?: string };
  isReference?: boolean; // Add this line
  externalUrl?: string;
  fileSize?: string;
  uploadedAt?: string;
  children?: Resource[];
  ...
  blobUrl?: string; // For extracted ZIP files
}
...
interface ResourceFilter {
  type: "video" | "pdf" | "ppt" | "folder" | "link" | "zip" | "reference";
  label: string;
  icon: any;
  checked: boolean;
}
...
const styles = `
  ...
  .dropdown-select { width: 100%; padding: 0.5rem 0.75rem; border: 1px solid hsl(var(--border)); border-radius: var(--radius); background-color: hsl(var(--background)); color: hsl(var(--foreground)); font-size: 0.75rem; cursor: pointer; display: flex; justify-content: space-between; align-items: center; }
...
// Helper function to get link URL for tooltip
...
const getFileType = (
  ...
  // ZIP detection - make this more comprehensive
...
const getFileUrl = (
  ...
  if (fileUrl['720p']) return fileUrl['720p'];
  if (fileUrl['480p']) return fileUrl['480p'];
  if (fileUrl['360p']) return fileUrl['360p'];
  if (fileUrl['240p']) return fileUrl['240p'];
  if (fileUrl.base) return fileUrl.base;

  return '';
}

// Helper function to extract duration from file
const getFileDuration = (file: PedagogyFile): string => {
  const sizeNum = Number.parseInt(file.size || "0")
...
// Helper function to format sub-item name
  ...
  return folders.map((folder, index) => ({
    ...
    children: [
      ...(folder.files || []).map((file, fIndex) => ({
        ...
        fileSize: `${(Number.parseInt(file.size || "0") / (1024 * 1024)).toFixed(1)} MB`,
      })),
...
// Helper function to calculate total duration for a module
      ...
      if (subModule.topics) {
        subModule.topics.forEach((topic) => {
          if (topic.duration) {
            ...
            totalMinutes +=
              (hoursMatch ? Number.parseInt(hoursMatch[1]) * 60 : 0) +
              (minutesMatch ? Number.parseInt(minutesMatch[1]) : 0)
          }
  ...
  if (module.topics) {
    module.topics.forEach((topic) => {
      if (topic.duration) {
        ...
        const minutesMatch = duration.match(/(\d+)m/)
        totalMinutes +=
          (hoursMatch ? Number.parseInt(hoursMatch[1]) * 60 : 0) + (minutesMatch ? Number.parseInt(minutesMatch[1]) : 0)
      }
    })
  }
...
// Check if item has child items that can be expanded
...
// Get child items count for display
...
// Check if item has pedagogy data (is a leaf node)
...
// Check if item should show arrow (has children in the data)
...
export default function LMSPage() {
  ...
  const [resourceFilters, setResourceFilters] = useState<ResourceFilter[]>([
    ...
    { type: "reference", label: "References", icon: FileText, checked: true }, // Add this line
  ])
  ...
  // NEW: Store root filters separately to preserve them when navigating folders
  ...
  // NEW: Effect to handle filter state when entering/leaving folders
  useEffect(() => {
    if (currentFolder) {
  ...
  const getAvailableResourceTypes = (): ResourceFilter["type"][] => {
    if (!selectedActivityData) return []

    const availableTypes: ResourceFilter["type"][] = []
  ...
  const toggleResourceFilter = (type: ResourceFilter["type"]) => {
    setResourceFilters(prev =>
      prev.map(filter =>
        filter.type === type
  ...
  const getSelectedFilters = (): ResourceFilter["type"][] => {
    return resourceFilters.filter(filter => filter.checked).map(filter => filter.type)
  }

  // NEW: Get available resource types for current folder content
  const getAvailableResourceTypesForCurrentFolder = (): ResourceFilter["type"][] => {
  ...
  const handleItemSelect = (
    itemId: string,
    itemTitle: string,
    itemType: "module" | "submodule" | "topic" | "subtopic",
    hierarchy: string[],
    pedagogy?: Pedagogy,
  ) => {
  ...
  const getResourcesByType = (
    type: "video" | "ppt" | "pdf" | "folder" | "link" | "zip"
  ): Resource[] => {
  ...
  const extractZipContents = async (resource: Resource): Promise<Resource[]> => {
    try {
      setExtractingZip(resource.id);

      const fileUrl = getFileUrl(resource.fileUrl);
      if (!fileUrl) {
        throw new Error('No file URL available for ZIP extraction');
      }
      ...
      const filePromises = Object.entries(zip.files).map(async ([relativePath, zipEntry]) => {
        if (zipEntry.dir) {
          // It's a directory - create folder resource
          extractedResources.push({
            id: `${resource.id}-dir-${relativePath}`,
            title: relativePath.split('/').filter(Boolean).pop() || relativePath,
            type: "folder",
            children: [] // Will be populated by files in this directory
          });
        } else {
          // It's a file - extract and determine type
          ...
          let fileType: Resource['type'] = 'link';
          ...
          extractedResources.push({
            id: `${resource.id}-file-${relativePath}`,
            title: fileName,
            type: fileType,
            fileSize: `${(content.size / (1024 * 1024)).toFixed(1)} MB`,
            blobUrl: blobUrl,
  ...
  // Replace the handleResourceClick function
  ...
  const renderResourceIcon = (type: string) => {
    switch (type) {
      ...
      case "reference":
        return <FileText className="w-3 h-3" /> // Or any icon you prefer for references
      default:
  ...
  const learningMethodOptions: DropdownOption[] = learningElements().map(element => ({
    value: element.id,
    label: element.title,
  ...
  // Get activity options based on selected method
  ...
  const getAvailableResourceTypesForCurrentContext = (): ResourceFilter["type"][] => {
    if (currentFolder) {
    ...
    <div className={`lms-container ${isDarkMode ? "dark" : ""}`}>
     ...
     =========================== */}
                    <div className="flex flex-col gap-2 mt-4">
                      ...
                      >
                        <div className="dropdown-grid grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="dropdown-group">
                            ...
                            <Dropdown
                              options={[
                                { value: "i-do", label: "I Do" },
                                { value: "we-do", label: "We Do" },
                                { value: "you-do", label: "You Do" },
                              ]}
                            ...
                            <Dropdown
                              options={
                                selectedMethod
                                  ? learningElements()
                                    .find((el) => el.id === selectedMethod)
                                    ?.subItems.map((s) => ({ value: s.key, label: s.name })) || []
                                  : []
                              }
                      ...
                      {selectedMethod &&
                        selectedActivity &&
                        (learningElements().find((el) => el.id === selectedMethod)?.subItems?.length ?? 0) > 0 && (
                              ...
                              <div className="text-center py-6 text-gray-500">
                                ...
                                <p className="text-xs">
                                  {getSelectedFilters().length === 0
                                    ? "Select at least one resource type to view resources"
                                    : "Try selecting different resource types or check back later for new content"}
                                </p>
                              </div>
                            )}
                          </div>
