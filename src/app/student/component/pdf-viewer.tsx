...
const controlsTimeoutRef = useRef<NodeJS.Timeout | undefined>(undefined);
  ...
  const handleSearch = () => {
    // PDF search functionality would be implemented here
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case 'Escape':
          e.preventDefault()
          if (notesOpen || aiOpen || sidebarOpen) {
            setNotesOpen(false)
            setAiOpen(false)
  ...
  }, [isOpen, notesOpen, aiOpen, isFullscreen, sidebarOpen])
  ...
  return (
    ...
    >
      {/* Main PDF Area */}
      <div className="flex flex-col h-screen bg-gray-800 relative w-full">
        {/* Top Navigation */}
        <div
          id="top-nav"
     ...
     transition-all duration-300 hover:shadow-xl px-3 sm:px-5 py-2 sm:py-3"
        ...
        >
          <div
            ...
            className="rounded-2xl overflow-hidden shadow-2xl border border-gray-300 hover:shadow-[0_0_40px_rgba(0,0,0,0.15)] transition-all duration-500"
          >
            <iframe
              ref={iframeRef}
              src={pdfUrl}
              title={title}
              className="bg-white shadow-2xl border-none rounded-2xl"
              style={{
                width: `${Math.min(window.innerWidth, 1400)}px`,
                height: `${Math.min(window.innerHeight - 80, 1000)}px`,
                transition: 'transform 0.2s ease',
                transform: `rotate(${rotation}deg) scale(${zoom})`,
                transformOrigin: 'center center',
              }}
              ...
              onError={(e) => {
                ...
                // If PDF fails to load, try opening in new tab
        ...
        >
          ...
          <div className="mt-8 w-full">
            ...
            <div className="flex items-center gap-4 flex-wrap bg-white/60 rounded-2xl p-3 shadow-inner hover:shadow-lg transition-all duration-300">
              ...
              {/* <button className="p-3 rounded-xl hover:bg-blue-100 active:scale-95 transition-all shadow-sm cursor-pointer" title="Bookmark">
